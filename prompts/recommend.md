...
1. **推荐逻辑 1**：
   - **课程推荐**：
     - 根据用户的`remaining_tasks`中的`credit_gaps`字段，优先推荐用户需要补足学分的个性化选修课程类别。
     - 在推荐课程时，优先选择学分高、难度适中且与用户知识点维度匹配的课程。
   - **科研推荐**：
     - 根据用户的`skills`字段，优先推荐能够提升用户薄弱能力的科研项目。
     - 结合`research.json`中的`skills`字段，选择能够显著提升用户能力的项目。
   - **竞赛推荐**：
     - 根据用户的目标（如保研或就业），优先推荐与目标相关的竞赛。
     - 如果用户目标是保研，优先推荐能够增加保研加分的竞赛。

2. **推荐逻辑 2**：
   - **课程推荐**：
     - 根据用户的`knowledge`字段，优先推荐能够提升用户薄弱知识点的个性化选修课程。
     - 结合课程的`knowledge`字段，选择能够补足用户知识短板的课程。
   - **科研推荐**：
     - 根据用户的`current_semester`和`enrollment_year`，筛选当前学期可选的科研项目。
     - 优先推荐周期较短、难度适中的科研项目。
   - **竞赛推荐**：
     - 根据用户的`skills`字段，优先推荐能够提升用户薄弱能力的竞赛。
     - 结合`contests.json`中的`skills`字段，选择能够显著提升用户能力的竞赛。
...
3. **推荐逻辑 3**：
   ...
   - **科研推荐**：
     - 根据用户的目标（如保研或就业），推荐与目标相关的科研项目。
     - 如果用户目标是保研，优先推荐能够增加科研成果（如发表论文）的项目。
   - **竞赛推荐**：
     - 根据用户的`current_semester`和竞赛的`duration`字段，筛选当前学期可参与的竞赛。
     - 优先推荐用户未参与过的竞赛，避免重复。

4. **推荐逻辑 4**：
   - **课程推荐**：
     - 综合考虑用户的`knowledge`、`skills`和`remaining_tasks`，推荐能够平衡提升知识点和能力的课程。
     - 在个性化选修课程中，优先推荐与用户目标相关的课程。
   - **科研推荐**：
     - 根据用户的`skills`字段，推荐能够提升用户薄弱能力的科研项目。
     - 优先推荐与用户目标相关的科研项目。
   - **竞赛推荐**：
